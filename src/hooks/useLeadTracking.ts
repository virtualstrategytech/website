...
export const useLeadTracking = (config?: CRMIntegration) => {
  ...
  const trackLead = useCallback(
    async (leadData: Omit<LeadData, "timestamp">) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to track lead";
  ...
  const sendToHubSpot = async (
    leadData: LeadData,
    hubspotConfig: NonNullable<CRMIntegration["hubspot"]>,
  ) => {
  ...
  const sendToCalendly = async (
    leadData: LeadData,
    calendlyConfig: NonNullable<CRMIntegration["calendly"]>,
  ) => {
    // In production, this would integrate with Calendly API
    console.log("Sending to Calendly:", leadData, calendlyConfig);
  };

  const sendToCalCom = async (
    leadData: LeadData,
    calcomConfig: NonNullable<CRMIntegration["calcom"]>,
  ) => {
